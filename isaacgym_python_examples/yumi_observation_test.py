...
#     joint_transforms = gym.get_actor_joint_transforms(envs[j], yumi_handles[j])
...
while True:
    ...
    print("hand pos:")
    print(rb_states[hand_idxs, :3])
    print(rb_states[hand_idxs, :3].size())
    # quat = gymapi.Quat(rb_states[hand_idxs[0], 3:7]).to_euler_zyx()
    euler = R.from_quat(rb_states[hand_idxs, 3:7].cpu())
    print("euler:")
    print(euler)
    euler1 = euler.as_euler('XYZ', degrees=False)   # intrinsic rotations
    euler2 = euler.as_euler('xyz', degrees=False)   # extrinsic rotations
    print("euler intrinsic", euler1, "euler extrinsic", euler2)
    # print(quat)
    # print(hand_pose.r)
