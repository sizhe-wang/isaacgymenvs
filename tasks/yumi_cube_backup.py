...
class YumiCube(VecTask):
    ...
    def __init__(self, cfg, sim_device, graphics_device_id, headless):
        ...
        self.cfg["env"]["numObservations"] = 525
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # yumi_dof_stiffness = to_torch([400, 400, 400, 400, 400, 400, 400, 1.0e4, 1.0e4], dtype=torch.float, device=self.device)
        yumi_dof_damping = to_torch([10, 10, 10, 10, 10, 10, 10, 1.0e2, 1.0e2], dtype=torch.float, device=self.device)
        # yumi_dof_damping = to_torch([80, 80, 80, 80, 80, 80, 80, 1.0e3, 1.0e3], dtype=torch.float, device=self.device)
    ...
    def compute_reward(self):
        ...
        height = self.grasp_height - self.table_dims.z
        rewards = torch.zeros(self.num_envs, device=self.device)
        rewards += height * self.height_reward_scale
        # reward if success
    ...
    def pre_physics_step(self, actions):
        # actions: delta num_envs * [x, y, z, rx, ry, rz]
        # do actions
        #   action:
        ...
        # =================================================================================
        # gripper_cmd [0,1] [0, 0.5]close:-=0.0025 [0.5, 1]open:+=0.0025
        gripper_actions = torch.where(operational_pos[:, 4].view(self.num_envs, 1) < 0.5,
        ...
        # count close
        self.gripper_close = torch.where(operational_pos[:, 4].view(self.num_envs, 1) < 0.5,
        ...
        # print("self.grasp_height", self.grasp_height)
        # print("self.gripper_close", self.gripper_close)
        # print(self.action_gripper_pos[:, 2])
