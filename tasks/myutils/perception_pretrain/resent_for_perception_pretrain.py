...
class ResNet(nn.Module):
    def __init__(self, layers=18, expansion=1, num_classes=10, learning_rate=1e-4, pretrained=False):
        ...
        # self._network = torch.nn.Sequential(*(list(eval('models.resnet{}'.format(layers))(pretrained=pretrained).children())[:-1]))
    ...
    def train_network(self, input_data=None, target=None, i=0):
        # print("--------------------------------------")
        # print("input", input_data[0], input_data.shape)
...
if __name__ == "__main__":
    net = ResNet18(num_classes=6, pretrained=False).cuda()
